...
public class AlertsTextHandler : MonoBehaviour
{
    ...
    // The backplates to use
    public GameObject defaultBP;
    public GameObject errorBP;
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            if (numErrors + numWarnings > 0)
            {
                ...
                if (numErrors == 0) {
                    ...
                    // Make sure the default backplate is active
                    errorBP.gameObject.SetActive(false);
                    defaultBP.gameObject.SetActive(true);

                    alertTextList[i].text = "Multiple warnings detected. Please check telemetry stream.";
                    return;
                }
                ...
                if (numWarnings == 0) {
                    ...
                    // Change the backplate to the error plate 
                    defaultBP.gameObject.SetActive(false);
                    errorBP.gameObject.SetActive(true);

                    alertTextList[i].text = "Multiple errors detected. Please check telemetry stream.";
                    return;
                }

                // At least one warning and at least one error
                else {
                    ...
                    alertTextList[i].text = "Multiple errors and warnings detected. Please check telemetry stream.";
                    return;
                }
